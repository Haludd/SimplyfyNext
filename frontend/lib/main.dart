...
class AppShell extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, _) => ConversationShell(
        room: room,
        appController: controller,
        signerView: SafeArea(
          top: false,
          child: LiveTranslatorScreen(controller: controller),
        ),
      ),
    );
  }
            ...
            children: <Widget>[
              ...
              if (cameraReady) ...<Widget>[
                ...
                Container(
                  ...
                  child: IconButton(
                    key: const ValueKey<String>('switch-camera-facing'),
                    onPressed: () => controller.switchCamera(),
                    tooltip:
                        'Use ${controller.devices.cameraFacing == CameraFacing.front ? 'back' : 'front'} camera',
                    icon: const Icon(
        ...
        content: ConstrainedBox(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              const SizedBox(height: 9),
              const Text(
                'The backend receives only this explicitly committed word sequence.',
                style: TextStyle(color: _yellow, fontSize: 10, height: 1.35),
              ),
            ],
          ),
    ...
    child: Column(
      ...
      children: <Widget>[
        ...
        _SettingLine(
          label: 'Camera source',
          value:
              '${controller.devices.cameraFacingLabel} · ${controller.devices.cameraName}',
          icon: Icons.videocam_outlined,
        ),
        _SettingLine(
          label: 'Camera status',
        ...
        Row(
          children: <Widget>[
            ...
            const SizedBox(width: 8),
            IconButton(
              key: const ValueKey<String>('settings-switch-camera-facing'),
              onPressed: () => controller.switchCamera(),
              icon: const Icon(Icons.cameraswitch_outlined, color: _mint),
              tooltip:
                  'Use ${controller.devices.cameraFacing == CameraFacing.front ? 'back' : 'front'} camera',
            ),
            IconButton(
