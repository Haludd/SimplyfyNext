...
final class LandmarkStreamWebSocketClient {
    ...
    while (true) {
      ...
      switch (event['type']) {
        case 'activity':
          ...
          if (event['state'] == 'signing' || event['state'] == 'processing') {
            continue;
          }
  ...
  Future<Map<String, dynamic>> ping({int? clientMs}) async {
    ...
    return _sendControlAndWait(
      ...
      (event) => event['type'] == 'pong' && event['control_seq'] == controlSeq,
    );
  }
  ...
  void _acceptDone() {
    ...
    if (!_closed) {
      ...
      for (final waiter in List<_EventWaiter>.from(_waiters)) {
        if (error != null) {
          waiter.completeError(error, _streamStackTrace ?? StackTrace.current);
        } else {
  ...
  LandmarkStreamProtocolException _serverError(Map<String, dynamic> event) =>
      LandmarkStreamProtocolException(
        ...
        message:
            event['message'] as String? ?? 'The backend returned an error.',
