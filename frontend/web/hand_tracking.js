...
// points cross into Flutter for local rendering and recognition.
...
// old opt-in DeepFace upload is deliberately disabled. The active room path
// sends only finalized English words and confidence scores.
...
async function start() {
  ...
  try {
    ...
    video.style.transform = preferredFacingMode === 'user' ? 'scaleX(-1)' : 'none';
...
async function setFacingMode(value) {
  if (value !== 'user' && value !== 'environment') {
    throw new Error('Camera facing mode must be user or environment.');
  }
  preferredFacingMode = value;
  for (const element of cameraElements()) {
    element.style.transform = value === 'user' ? 'scaleX(-1)' : 'none';
  }
}
