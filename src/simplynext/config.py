...
class Settings(BaseSettings):
    ...
    # Direct Gemini API mode. The adapter reads GEMINI_API_KEY from the process
    # environment, never from application settings or client payloads.
    ...
    # The REST adapter uses one accounted dispatch per guarded call.
    ...
    # Local demos can admit recognizer labels outside a reviewed policy vocabulary.
    # Production always requires vocabulary membership.
    word_policy_enforce_vocabulary: bool = True
    # Local integration tests may send provisional recognizer scores to the
    # sentence model. Production always enforces the reviewed score threshold.
    ...
    def require_bedrock_ownership_and_budget_headroom(self) -> Self:
        ...
        if self.gemini_enabled:
            ...
            if any(rate is None for rate in gemini_pricing):
                raise ValueError("all four gemini pricing fields are required when Gemini is enabled")
        pricing_fields = {
